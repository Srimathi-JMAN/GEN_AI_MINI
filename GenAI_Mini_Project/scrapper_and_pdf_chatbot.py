...
def chunk_text(text, chunk_size=500):
    return [text[i:i + chunk_size] for i in range(0, len(text), chunk_size-20)]

def update_embeddings(chat_id, text_chunks):
    cursor.execute("DELETE FROM embeddings WHERE chat_id = %s;", (str(chat_id),))
    conn.commit()

    cursor.execute("""SELECT setval('embeddings_id_seq', (SELECT MAX(id) FROM embeddings));""")
    conn.commit()
...
for chat_id in st.session_state.chats.keys():
    if st.sidebar.button(f"Chat {str(chat_id)[:8]}"):
...
# Main UI
st.title("Web Scraper and PDF Chatbot 👤")
